...
async function loadBlogPost() {
    ...
    try {
        ...
        // Load post content
        const contentResponse = await fetch(`posts/${postSlug}.md`);
        const markdown = await contentResponse.text();
        
        // Parse metadata and content
        const { metadata, content } = parseMetadata(markdown);
        
        // Use metadata if available, otherwise use posts.json
        const title = metadata.title || postMeta.title;
        const date = metadata.date || postMeta.date;

        // Remove first H1 to avoid duplication
        const contentWithoutTitle = removeFirstH1(content);

        // Update page
        document.getElementById('blog-title').textContent = title;
        document.getElementById('blog-date').textContent = date;
        document.getElementById('blog-content').innerHTML = parseMarkdown(contentWithoutTitle);
        document.title = `${title} - Miles`;
    } catch (error) {
